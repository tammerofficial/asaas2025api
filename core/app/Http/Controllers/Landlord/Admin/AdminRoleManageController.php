...
//        $role->syncPermissions($request->permission);
        ...
        $role->syncPermissions($permissions); // Pass model collection, not IDs
...
//    forbidden-subdomain
        public function forbiddenSubdomain(Request $request)
        {
            return view('landlord.admin.forbidden-subdomain.forbidden-subdomain');
        }
        public function storeForbiddenSubdomain(Request $request)
        {
//            $request->validate([
//                'forbidden_subdomains' => ['nullable','regex:/^[a-z0-9,\-]+$/i'],
//            ]);

            if ($request->has('forbidden_subdomains')) {
                update_static_option('forbidden_subdomains', strtolower($request->forbidden_subdomains));
                return redirect()->back()->with(['msg' => __('Forbidden Subdomains updated..'),'type'=> 'success']);
            }else{
                return redirect()->back()->with(['msg' => __('Forbidden Subdomains Create Failed!'),'type'=> 'danger']);
            }
