    ...
    private function invoice_design($payment_details)
    {
        ...
        if ($tenant)
        {
            ...
            $description .= '<p>Expire Date: ' .
                ($tenant->expire_date ? \Illuminate\Support\Carbon::parse($tenant->expire_date)->format('d/m/Y') : __('Lifetime')) .
                '</p>';
        }
    ...
    public function payment_log_payment_status_change($id)
    {
        try {
            ...
            if ($payment_log->payment_status == 'complete') {
                ...
                // Get OLD payment log for this tenant (the last completed one before this)
                if ($payment_log->tenant_id) {
                    $old_payment_log = PaymentLogs::where('tenant_id', $payment_log->tenant_id)
                        ->where('user_id', $payment_log->user_id)
                        ->where('payment_status', 'complete')
                        ->latest('id')
                        ->first();

                    if ($old_payment_log && $old_payment_log->expire_date) {
                        $renewal_after_expire_date = Carbon::parse($old_payment_log->expire_date)->isFuture() ? 0 : 1;
                    }
                }

                $payment_log->order_id = $payment_log->id;
                (new PaymentGateways())->update_tenant($payment_log , $renewal_after_expire_date);
