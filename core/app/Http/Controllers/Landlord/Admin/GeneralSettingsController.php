...
//            case "queue":
    ...
    public function update_database_upgrade(Request $request)
    {
        try {
            ...
            // 1️⃣ Run landlord migrations and seeders
            Artisan::call('migrate', ['--force' => true]);
            Artisan::call('db:seed', ['--class' => UpdateInfoSeeder::class, '--force' => true]);
            Artisan::call('db:seed', ['--force' => true]);

            // 2️⃣ Run tenant migrations and tenant-specific seeders
            Tenant::latest()->chunk(50, function ($tenants) {
                foreach ($tenants as $tenant) {
                    try {
                        ...
                        // 🧩 Run SlugModifierSeeder if exists
                        if (class_exists('Database\Seeders\SlugModifierSeeder')) {
                            Artisan::call('tenants:seed', [
                                '--class' => \Database\Seeders\SlugModifierSeeder::class,
                                '--force' => true,
                                '--tenants' => $tenant->id
                            ]);
                        }

                        // 🆕 Run PermissionSyncSeeder to safely add missing permissions
                        if (class_exists('Database\Seeders\Tenant\PermissionSyncSeeder')) {
                            Artisan::call('tenants:seed', [
                                '--class' => \Database\Seeders\Tenant\PermissionSyncSeeder::class,
                                '--force' => true,
                                '--tenants' => $tenant->id
                            ]);
                            Log::info("✅ Synced permissions for tenant ID {$tenant->id}");
                        }

                    } catch (\Exception $e) {
                        Log::error("❌ Tenant migrate or seed failed for ID {$tenant->id}: " . $e->getMessage());
                    }
                }
            });

            // 3️⃣ Backfill tenant renewal data
            try {
                Artisan::call('app:backfill-tenant-renewal-data');
                Log::info('✅ Tenant renewal backfill command executed successfully.');
            } catch (\Exception $e) {
                Log::error('❌ Failed to run tenant backfill command: ' . $e->getMessage());
            }

            // 4️⃣ Sync tenant digital product permissions (command-based sync)
            try {
                Artisan::call('app:sync-tenant-permissions');
                Log::info('✅ Tenant digital product permission sync executed successfully.');
            } catch (\Exception $e) {
                Log::error('❌ Failed to sync tenant permissions: ' . $e->getMessage());
            }
            ...
            return redirect()->back()->with([
                'msg' => __('Database upgrade and synchronization completed successfully.'),
                'type' => 'success'
            ]);

        } catch (\Exception $e) {
            Log::error('❌ Database upgrade failed: ' . $e->getMessage());

            return redirect()->back()->with([
                'msg' => __('Database upgrade failed. Check logs for details.'),
                'type' => 'danger'
            ]);
        }
    }
