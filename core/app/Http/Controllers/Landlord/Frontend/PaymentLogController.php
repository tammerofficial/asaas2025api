...
class PaymentLogController extends Controller
{
    ...
    {
        $manual_transection_condition = $request->selected_payment_gateway === 'manual_payment'
            ? 'required'
            : 'nullable';
        ...
        $old_completed_log = PaymentLogs::where('user_id', $auth_id)
            ->where('tenant_id', $subdomain) // theme_slug is required
        ...
        try {
            ...
            Log::info('Creating new payment log', [
                'user_id' => $auth_id,
                'tenant_id' => $subdomain,
            ...
            Log::info('Payment log created successfully', [
                'log_id' => $this->payment_details->id,
                'is_renewal' => $is_renewal
            ]);
