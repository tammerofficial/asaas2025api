...
use App\Http\Requests\Api\Central\SupportTicket\UpdateSupportTicketRequest;
use App\Http\Resources\Api\Central\SupportTicketResource;
...
class SupportTicketController extends BaseApiController
{
    ...
    {
        try {
            $cached = $this->remember('central_support_tickets_' . request()->get('page', 1), 300, function () {
    ...
    {
        try {
            $cached = $this->remember("central_support_ticket_{$supportTicket->id}", 600, function () use ($supportTicket) {
    ...
    public function update(UpdateSupportTicketRequest $request, SupportTicket $supportTicket): JsonResponse
    {
        ...
        $this->clearCache('central_support_tickets*');
        $this->clearCache("central_support_ticket_{$supportTicket->id}*");
    ...
    public function addMessage(SupportTicket $supportTicket, \Illuminate\Http\Request $request): JsonResponse
    {
        ...
        $message = SupportTicketMessage::create(array_merge($validated, ['support_ticket_id' => $supportTicket->id, 'user_id' => auth('sanctum')->id(), 'type' => 'admin']));
        $this->clearCache("central_support_ticket_{$supportTicket->id}*");
