...
//            $cart_data = $request->all();
////            dd($cart_data);
//        $product_id = (int) $cart_data['product_id'];
////        dd($product_id);
...
//    }
    public function add_to_wishlist(Request $request): JsonResponse
    {
        ...
        // Get product first - THIS WAS THE MAIN ISSUE
        ...
        if (!empty($product->campaign_product)) {
            $sold_count = CampaignSoldProduct::where('product_id', $request->product_id)->first();
            $product_left = $sold_count !== null
                ? $product->campaign_product->units_for_sale - $sold_count->sold_count
                : $product->campaign_product->units_for_sale;
        } else {
            $product_left = $product_inventory_details->stock_count ?? $product_inventory->stock_count ?? 0;
        }

        // Check if requested quantity is available
        if ($request->quantity > $product_left) {
            return response()->json([
                'type' => 'warning',
                'quantity_msg' => __('Requested amount can not be added. Product stock limit is over!')
            ]);
        }
        ...
        try {
            ...
            $existingItem = Cart::instance("wishlist")->search(function ($cartItem, $rowId) use ($product_id, $request) {
                return $cartItem->id == $product_id &&
                    ($cartItem->options->variant_id ?? null) == ($request->product_variant ?? null);
            });

            if ($existingItem->isNotEmpty()) {
                return response()->json([
                    'type' => 'info',
                    'msg' => __('Item already exists in wishlist')
                ]);
            }
            ...
            Log::error('Wishlist Error: ' . $exception->getMessage());
...
//        }
        ...
        $shipping_tax_markup = themeView('shop.checkout.markup_for_controller.shipping_tax_ajax', compact('shipping_tax', 'shipping_methods', 'default_shipping', 'country', 'city'))->render();
...
//    public function ajax_remove_cart_item(Request $request)
//    {
//        $request->validate([
...
//        \Gloudemans\Shoppingcart\Facades\Cart::instance($instance)->remove($request->row_id);
//
//        $count = \Gloudemans\Shoppingcart\Facades\Cart::instance($instance)->count();
//
//        return response()->json([
//            'success' => true,
//            'count' => $count,
//        ]);
//    }
    public function ajax_remove_cart_item(Request $request): JsonResponse
    {
        $request->validate([
            'row_id' => 'required|string',
            'instance' => 'required|string|in:default,wishlist'
        ]);
        ...
        try {
            ...
            // Get user ID for cart persistence
            $username = Auth::guard("web")->check() ? Auth::guard("web")->user()->id : null;

            // Restore the cart instance
            if ($username) {
                Cart::instance($instance)->restore($username);
            }
            ...
            if ($username) {
                Cart::instance($instance)->store($username);
            }

            // Get updated count
            $count = Cart::instance($instance)->count();

            $itemType = $instance === 'wishlist' ? 'wishlist' : 'cart';
//dd($count, $instance);
            return response()->json([
                'success' => true,
                'message' => __("Item removed from $itemType successfully"),
                'count' => $count,
                'instance' => $instance
            ]);
