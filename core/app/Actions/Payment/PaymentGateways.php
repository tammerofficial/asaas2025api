...
//            $payment_log = PaymentLogs::where('id', $payment_data['order_id'])->first();
//            if (!$payment_log) {
////                Log::error('Payment log not found for order_id: ' . $payment_data['order_id']);
//                return;
//            }
//
//            $tenant = Tenant::find($payment_log->tenant_id);
//            if (!$tenant) {
////                Log::error('Tenant not found for tenant_id: ' . $payment_log->tenant_id);
//                return;
//            }
//
...
////                    Log::info('Skipping tenant update for ID ' . $tenant->id . ': Recently updated');
//                    return;
//                }
//
...
////                Log::info('Calculated expire_date for tenant ' . $tenant->id . ': ' . $expire_date);
//
//                DB::beginTransaction();
//                DB::table('tenants')->where('id', $tenant->id)->lockForUpdate()->update([
//                    'renew_status' => is_null($tenant->renew_status) ? 0 : $tenant->renew_status + 1,
...
//                DB::commit();
//
////                Log::info('Tenant updated: ' . json_encode(Tenant::find($tenant->id)));
//
