...
//    }
    private static function smsSender($user)
    {
        $otp = new OtpTraitService();

        if (get_static_option('new_tenant_user')) {
            $otp->sendSms([$user->mobile, __('Hello, Your new shop is created successfully - ' . get_static_option('site_title'))]);
        }

        if (get_static_option('new_tenant_admin')) {
            $otp->sendSms([get_static_option('receiving_phone_number'), __('A new shop has been created - ' . get_static_option('site_title'))]);
        }
    }
