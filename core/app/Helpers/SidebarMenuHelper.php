...
class SidebarMenuHelper
{
    ...
    public function render_sidebar_menus(): string
    {
        ...
        if (isPluginActive('CommissionManage')){
            $this->commission_manage_settings_menus($menu_instance);
        }
...
// testimonial
    ...
    private function commission_manage_settings_menus(MenuWithPermission $menu_instance): void
    {
        $menu_instance->add_menu_item('commission-manage-settings-menu-items', [
            'route' => '#',
...
//            'permissions' => ['price-plan-list', 'price-plan-create', 'price-plan-edit', 'price-plan-delete','coupon-list'],
        ...
        $menu_instance->add_menu_item('commission-manage-settings-page', [
            'route' => 'landlord.admin.commission.settings',
            'label' => __('Commission Settings'),
            'parent' => 'commission-manage-settings-menu-items',
//            'permissions' => ['price-plan-list'],
        ]);
    }
...
//        ]);
    ...
    {
//         $current_tenant_payment_data = tenant()->payment_log ?? []; // pricing_plan_id => 13   (old)
        $current_tenant_payment_data = PaymentLogs::where('id', tenant()->renewal_payment_log_id)->first() ?? []; // pricing_plan_id => 1 (new)
//        dd($current_tenant_payment_data);
        $admin = Auth::guard('admin')->user();
...
//        ]);
    ...
    private function tenant_product_settings_menus(MenuWithPermission $menu_instance): void
    {
        ...
        $menu_instance->add_menu_item('product-import-export-items', [
            'route' => 'tenant.products.import',
            'label' => __('CSV Import/Export'),
            'parent' => 'product-settings-menu-items',
            'permissions' => [],
//            'class' => 'submenu-disabled'
        ]);
    ...
    private function tenant_digital_product_settings_menus(MenuWithPermission $menu_instance): void
    {
        $menu_instance->add_menu_item('digital-product-settings-menu-items', [
            ...
            'parent' => null,
            'permissions' => [
                'digital_product','digital-product-type-list', 'digital-product-list', 'digital-product-create', 'digital-product-edit', 'digital-product-delete', 'digital-product-settings', 'digital-product-reviews',
