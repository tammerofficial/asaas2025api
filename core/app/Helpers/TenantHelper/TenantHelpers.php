...
//            'assign_status' => 1
    ...
    {
        //todo:: this will return all the information related to tenant_id
        $tenant_id = $this->getTenant();
//        dd($tenant_id);
        $tenant_details = Tenant::where('id', $tenant_id->id)->first();
//        dd($tenant_details);
        $user_details = User::where('id', $tenant_details->user_id)->first();
//        dd($user_details);
        $shop_info =  $this->getShopURL($user_details,$tenant_details);
//        dd($shop_info);
        $return_data = collect([
            'tenant_id' => $tenant_id,
            'theme_slug' => $tenant_details->theme_slug,
            'expire_date' => $tenant_details->expire_date,
            'start_date' => $tenant_details->start_date,
            'shop_url' => $shop_info['final_url'],
            'shop_url_with_login_token' => $shop_info['final_url_with_login_hash'],
            'payment_logs' => $tenant_details->payment_log,
        ]);

        return $return_data;
    }

    public function getShopURL($user_details,$tenant_details)
    {
       $url = '';
      $central = '.'.env('CENTRAL_DOMAIN');

    if(!empty($tenant_details->custom_domain?->custom_domain) && $tenant_details->custom_domain?->custom_domain_status == 'connected'){
        $custom_url = $tenant_details->custom_domain?->custom_domain ;
        $url = tenant_url_with_protocol($custom_url);
    }else{
        $local_url = $this->getTenant() .$central ;
        $url = tenant_url_with_protocol($local_url);
    }

    $hash_token = hash_hmac('sha512',$user_details->username.'_'.$tenant_details->id, $tenant_details->unique_key);
    $final_url = tenant_url_with_protocol(optional($tenant_details->domain)->domain);
    $final_url_with_login_hash = $url.'/token-login/'.$hash_token;

    return [
            'final_url_with_login_hash' => $final_url_with_login_hash,
            'final_url' => $final_url,
        ];
    }
...
//        $this->paymentLogHistory = PaymentLogHistory::create($finalData);;
    ...
    public function sendEmailToUserForOrderAdminApprovalNotice()
    {
        try {
            ...
            Mail::to(get_static_option_central('site_global_email'))->send(new BasicMail($admin_message, $admin_subject));
    ...
    {
        return Carbon::now()->endOfDay()->format('d-m-Y H:i:s');
    }
...
//    }
    private function getEndDateForMonthlyPlan()
    {
        ...
        $packageLog = $this->getPaymentLog();
        $date = $this->isRenew() && $packageLog ? Carbon::parse($packageLog->expire_date)->startOfDay() : Carbon::now()->startOfDay();
        return $date->addMonth(1)->endOfDay()->format('d-m-Y H:i:s');
    }
...
//    }
    private function getTrialEndDate()
    {
        return Carbon::now()->addDays($this->getPackage()->trial_days)->endOfDay()->format('d-m-Y H:i:s');
    }
    ...
    private function getEndDateForYearlyPlan()
    {
        ...
        $date = $this->isRenew() && $paymentLogs ? Carbon::parse($paymentLogs->expire_date)->startOfDay() : Carbon::now()->startOfDay();
        return $date->addYear(1)->endOfDay()->format('d-m-Y H:i:s');
    }

    private function getEndDateForCustomPlan()
    {
        $custom_expire_date = request()->custom_expire_date;
        return Carbon::parse($custom_expire_date)->endOfDay()->format('d-m-Y H:i:s');
    }
    ...
    public function getExpiredDate($absoluteDate = false)
    {
        ...
        if (!$packageDetails) {
            Log::error('Package not found for tenant: ' . $this->getTenantId());
            return null;
        }
        ...
        if ($absoluteDate) {
            $paymentLogs = $this->getPaymentLog();
//            dd($paymentLogs);
            return $paymentLogs && $paymentLogs->expire_date ? Carbon::parse($paymentLogs->expire_date)->format('d-m-Y H:i:s') : null;
        }
        ...
        $renewCalculationMethod = get_static_option('renew_date_calculation') ?? 'append_to_old';
//        dd($paymentLogs);
        $tenant = $this->getTenant();
...
//dd($this);
        ...
        if ( $renewCalculationMethod === 'append_to_old') {

            // For renewals, use the current expire_date from tenant table as base
            if ($tenant && $tenant->expire_date && Carbon::parse($tenant->expire_date)->isFuture()) {
                $baseDate = Carbon::parse($tenant->expire_date)->startOfDay();
            }
        }
        // Calculate new expire date based on package type
        $expireDate = match ($packageDetails->type) {
            0 => $baseDate->copy()->addMonth(1)->endOfDay()->format('d-m-Y H:i:s'), // Monthly
            1 => $baseDate->copy()->addYear(1)->endOfDay()->format('d-m-Y H:i:s'),  // Yearly
            3 => $this->getEndDateForCustomPlan(), // Custom
            default => null // Lifetime
        };
//dd($expireDate, $baseDate);
//        dd($baseDate);
//        Log::info('Calculated expire_date for tenant ' . $this->getTenantId() . ': ' . $expireDate . ', baseDate: ' . $baseDate->format('d-m-Y H:i:s') . ', isRenew: ' . ($this->isRenew() ? 'true' : 'false'));
    ...
    public function getTenantExpiredDate()
    {
        if (is_null($this->getTenant())) {
            return __('website not yet created');
        }
    ...
    public static function getTenantInfoForDisplay($user_id)
{
    $tenant = Tenant::where('user_id', $user_id)->first();
    $payment_log = PaymentLogs::where('tenant_id', $tenant?->id)->latest()->first();

    $start_date = $tenant?->start_date ? Carbon::parse($tenant->start_date)->format('d-m-Y H:i:s') : __('Not Set');
    $expire_date = $tenant?->expire_date ? Carbon::parse($tenant->expire_date)->format('d-m-Y H:i:s') : __('Lifetime');
    $is_renew = $tenant?->is_renew ;

    $status = 'pending';
    if ($payment_log) {
        if ($payment_log->status === 'trial') {
            $status = 'trial';
        } elseif ($payment_log->status === 'complete') {
            $status = $tenant && $tenant->expire_date && Carbon::parse($tenant->expire_date)->startOfDay()->lessThan(Carbon::today()) ? 'expired' : 'active';
        }
    }
