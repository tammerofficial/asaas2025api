<script>
    (function ($) {
        ...
        $(document).ready(function () {
            ...
            // Fix form actions for existing widgets on page load
            $('#header li form').each(function() {
                var type = $(this).find('input[name="addon_type"]').val();
                var action = type === 'new' 
                    ? "{{route(route_prefix().'admin.header.builder.store')}}" 
                    : "{{route(route_prefix().'admin.header.builder.update')}}";
            ...
            * ---------------------------------*/
            ...
            function setAddonLocation(event){
                var addonLocation = 'header'; // Force location to header
                var allDraggerdAddon = $('#header').find('li');
            ...
            function renderWidgetMarkup(event,li){
                ...
                var markup = '';
                $.ajax({
                    'url': "{{route(route_prefix().'admin.header.builder.get.addon.markup')}}",
                    'type': "POST",
                    'data': {
                        '_token': "{!! csrf_token() !!}",
                    ...
                    success: function (data) {
                        markup = data;
                        // Fix form action for header builder
                        markup = markup.replace(/admin\.page\.builder\.(new|update)/g, function(match, type) {
                            return type === 'new' ? 'admin.header.builder.store' : 'admin.header.builder.update';
                        });
                    }
                });
                ...
                li.item.clone()
                    .html(markup)
                    .insertAfter(li.item);
                // Fix form actions in rendered widget
                $('#header li').last().find('form').each(function() {
                    var type = $(this).find('input[name="addon_type"]').val();
                    var action = type === 'new' 
                        ? "{{route(route_prefix().'admin.header.builder.store')}}" 
                        : "{{route(route_prefix().'admin.header.builder.update')}}";
            ...
            $('body').on('click', '.remove-widget', function (e) {
                Swal.fire({
                    title: "{{ __('Do you want to remove this addon?') }}",
                    text: "{{__('If you remove this addon, all saved data associated with it will be lost.')}}",
                    icon: "warning",
                    ...
                    if (result.isConfirmed) {
                        ...
                        if (widgetType === 'update') {
                            ...
                            $.ajax({
                                'url': "{{route(route_prefix().'admin.header.builder.delete')}}",
                                'type': "POST",
                                'data': {
                                    '_token': "{!! csrf_token() !!}",
                                    'id': widget_id
            ...
            $('body').on('click', '.widget_save_change_button', function (e) {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        udpateId = data.id;
                        if (widgetType === 'new') {
                            formContainer.attr('action', "{{route(route_prefix().'admin.header.builder.update')}}");
            ...
            function resetOrder(dropedOn) {
                var allItems = $('#header li');
                $.each(allItems, function (index, value) {
                    $(this).find('input[name="addon_order"]').val(index + 1);
                    $(this).find('input[name="addon_location"]').val('header');
            ...
            function reset_db_order(id, addon_order) {
                $.ajax({
                    type: "POST",
                    url: "{{route(route_prefix().'admin.header.builder.update.order')}}",
                    data: {
                        _token: "{{csrf_token()}}",
                        id: id,
        ...
        $('#header-settings-form').on('submit', function(e) {
            ...
            // Get checkbox values (send 'yes' if checked, 'no' if unchecked)
            var stickyValue = $('#header_sticky').is(':checked') ? 'yes' : 'no';
            var transparentValue = $('#header_transparent').is(':checked') ? 'yes' : 'no';
            
            submitBtn.text('{{__('Saving...')}}').prop('disabled', true);
            
            $.ajax({
                type: "POST",
                url: "{{route(route_prefix().'admin.header.builder.save.settings')}}",
                data: {
                    _token: "{{csrf_token()}}",
                    header_sticky: stickyValue,
                    header_transparent: transparentValue
                },
                success: function (data) {
                    submitBtn.text('{{__('Saved!')}}');
                    if (data.msg) {
                        toastr.success(data.msg);
                    } else {
                        toastr.success('{{__('Settings saved successfully')}}');
                    }
                    setTimeout(function() {
