...
{{--                    : {{tenant()->expire_date?->diffForHumans() ?? ''}}--}}
                    @if(optional(tenant()->payment_log)->status == 'trial')
                        <strong class="text-capitalize"> ( {{optional(tenant()->payment_log)->status}} )</strong>
                    ...
                    @else
                        @if(tenant()->expire_date != null)
                            <strong> ( {{__('Expire Date :')}} {{tenant()->expire_date->format( 'd-m-Y') }} )</strong>
