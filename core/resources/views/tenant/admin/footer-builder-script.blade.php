<script>
    (function ($) {
        ...
        $(document).ready(function () {
            ...
            $('#footer li form').each(function() {
                var type = $(this).find('input[name="addon_type"]').val();
                var action = type === 'new' 
                    ? "{{route(route_prefix().'admin.footer.builder.store')}}" 
                    : "{{route(route_prefix().'admin.footer.builder.update')}}";
            ...
            * ---------------------------------*/
            ...
            function setAddonLocation(event){
                var addonLocation = 'footer'; // Force location to footer
                var allDraggerdAddon = $('#footer').find('li');
            ...
            function renderWidgetMarkup(event,li){
                ...
                var markup = '';
                $.ajax({
                    'url': "{{route(route_prefix().'admin.footer.builder.get.addon.markup')}}",
                    'type': "POST",
                    'data': {
                        '_token': "{!! csrf_token() !!}",
                    ...
                    success: function (data) {
                        markup = data;
                        // Fix form action for footer builder
                        markup = markup.replace(/admin\.page\.builder\.(new|update)/g, function(match, type) {
                            return type === 'new' ? 'admin.footer.builder.store' : 'admin.footer.builder.update';
                        });
                    }
                });
                ...
                $('#footer li').last().find('form').each(function() {
                    var type = $(this).find('input[name="addon_type"]').val();
                    var action = type === 'new' 
                        ? "{{route(route_prefix().'admin.footer.builder.store')}}" 
                        : "{{route(route_prefix().'admin.footer.builder.update')}}";
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        if (widgetType === 'update') {
                            ...
                            $.ajax({
                                'url': "{{route(route_prefix().'admin.footer.builder.delete')}}",
                                'type': "POST",
                                'data': {
                                    '_token': "{!! csrf_token() !!}",
                                    'id': widget_id
            ...
            $('body').on('click', '.widget_save_change_button', function (e) {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        udpateId = data.id;
                        if (widgetType === 'new') {
                            formContainer.attr('action', "{{route(route_prefix().'admin.footer.builder.update')}}");
            ...
            function resetOrder(dropedOn) {
                var allItems = $('#footer li');
                $.each(allItems, function (index, value) {
                    $(this).find('input[name="addon_order"]').val(index + 1);
                    $(this).find('input[name="addon_location"]').val('footer');
            ...
            function reset_db_order(id, addon_order) {
                $.ajax({
                    type: "POST",
                    url: "{{route(route_prefix().'admin.footer.builder.update.order')}}",
                    data: {
                        _token: "{{csrf_token()}}",
                        id: id,
