@php use App\Models\PaymentLogs; @endphp
                            ...
                            <tr>
                                ...
                                <td>
                                    <a class="badge rounded bg-primary px-4 visitweb"
                                       href="{{tenant_url_with_protocol(optional($data->domain)->domain)}}"
                                       target="_blank">{{__('Visit Website')}}</a>
                                    <a class="badge rounded bg-danger px-4" href="{{$url.'/token-login/'.$hash_token}}"
                                       target="_blank">{{__('Login as Super Admin')}}</a>
                                    {{--                                    <a href="javascript:void(0)" class="badge rounded bg-success px-4"--}}
                                    {{--                                       data-bs-toggle="modal"--}}
                                    {{--                                       data-bs-target="#user_add_subscription"--}}
                                    {{--                                    >{{__('Renew Shop')}}</a>--}}
                                </td>
    ...
    <div class="modal fade" id="user_add_subscription" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <form action="{{route('landlord.frontend.order.payment.form')}}" id="user_add_subscription_form"
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group coupon-wrapper" style="display: none">
                            ...
                            <div class="coupon-form d-flex gap-2" id="coupon-form">
                                <input class="form-control coupon" name="coupon" id="coupon" type="text"
                                       autocomplete="off" value="{{old('coupon')}}"
                        ...
                        <div class="form-group mt-3" style="display: none">
                            ...
                            <select class="form-select text-capitalize" name="theme_slug" id="custom-theme">
                                @foreach($themes as $theme)
                                    ...
                                    @endphp
                                    <option
                                        value="{{$theme}}" {{$theme === get_static_option('default_theme') ? 'selected' : ''}}>{{$custom_theme_name}}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" class="btn btn-primary" data-bs-target="#final_result"
                                id="submit-btn">{{__('Create')}}</button>
                    </div>
                </form>
            </div>
        </div>
...
@section('scripts')
    ...
    <script>
        ...
        $(document).on('change', 'select[name="package"]', function () {
            ...
            // let theme_slug = final_detail.theme;
            ...
            $.ajax({
                ...
                success: async function (data) {
                    ...
                    async function fetchOldTenantLog(userId, tenantId) {
                        try {
                            const token = document.querySelector('meta[name="csrf-token"]').getAttribute('content');

                            const res = await fetch('/payment-log', {
                                method: 'POST',
                                headers: {
                    ...
                    const userId = window.auth_id;
                    const oldTenantLog = await fetchOldTenantLog(userId, final_detail.theme);

                    async function fetchPackageDates(subdomain, plan, oldTenantLog, themeSlug) {
                        try {
                            const token = document.querySelector('meta[name="csrf-token"]').getAttribute('content');

                            const response = await fetch('/tenant-helper', {
                                method: 'POST',
                                headers: {
                                    'Content-Type': 'application/json',
                                    'Accept': 'application/json',
                                ...
                                body: JSON.stringify({subdomain, plan, oldTenantLog, themeSlug})
                            });

                            const data = await response.json();
                            // console.log(data);
                            return {
                                packageStartDate: data.start_date,
                                packageExpireDate: data.expire_date
                            };
                        } catch (err) {
                            return {packageStartDate: null, packageExpireDate: null};
                        }
                    }

                    let plan = parseInt(package_id, 10);
                    const {packageStartDate, packageExpireDate} = await fetchPackageDates(
                        subdomain, plan, oldTenantLog, final_detail.theme
                    );

                    final_detail.validity = packageExpireDate;
        ...
        $(document).on('click', 'button[data-bs-target="' + modal_id + '"]', function () {
            // console.log(final_detail.theme);
            ...
            // modal.find('#custom-theme').text(final_detail.theme);
