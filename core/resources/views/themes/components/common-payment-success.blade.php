...
{{--<tr>--}}
{{--    <td></td>--}}
{{--    <td class="font-weight-bold">{{__('Coupon Discount: (-)')}}</td>--}}
{{--    <td>--}}
{{--        @php--}}
{{--            $discountAmount = 0;--}}
{{--            $discountType = $payment_details?->product_coupon?->discount_type;--}}
{{--            $discountValue = $payment_details?->product_coupon?->discount;--}}
{{--            $subAmount = json_decode($payment_details->payment_meta)?->subtotal;--}}

{{--            if ($discountType === 'percentage' && $discountValue) {--}}
{{--                $discountAmount = ($subAmount * $discountValue) / 100;--}}
...
{{--        @if($discountAmount > 0)--}}
{{--            {{ amount_with_currency_symbol($discountAmount) }}--}}
{{--        @endif--}}
{{--    </td>--}}
            ...
            <div class="billing-wrappers">
                <div class="billing-items">
                    ...
                    <ul class="billing-details">
                        ...
                        <li>
                            <strong>{{__('Payment Type:')}}</strong> {{$payment_details->checkout_type == 'cod' ? __('Cash On Delivery') : __('Digital Payment')}}
                        </li>
                        ...
                        @if(!empty($payment_details->coupon))
                            <li>
                                <strong>{{__('Paid Amount After Discount :')}}</strong> {{ amount_with_currency_symbol(optional($payment_details->package)->price) }}
                            </li>
                        @endif
                ...
                <div class="billing-items">
                    ...
                    <ul class="billing-details">
                        ...
                        <li><strong>{{__('Email:')}}</strong> <span
                                class="text-lowercase">{{$payment_details->email}}</span></li>
                    </ul>
                    <div class="btn-wrapper margin-top-40">
                        @if(auth()->guard('web')->check())
                            <a href="{{route('tenant.user.home')}}"
                               class="boxed-btn rounded-0 btn btn-primary">{{__('Go To Dashboard')}}</a>
                        @else
                            <a href="{{route('tenant.frontend.homepage')}}"
                               class="boxed-btn rounded-0 btn btn-primary">{{__('Back To Home')}}</a>
                        @endif
                    </div>
                </div>
            </div>
                        ...
                        <div class="price-body price-table">
                            <table class="table">
                                ...
                                <tr>
                                    <td></td>
                                    <td class="font-weight-bold">{{__('Total:')}}</td>
                                    <td>{{amount_with_currency_symbol($payment_details->total_amount)}} <small
                                            class="vat-tax">{{__('(Incl TAX & Shipping)')}}</small></td>
                                </tr>
                                </tbody>
                            </table>
                        </div>
