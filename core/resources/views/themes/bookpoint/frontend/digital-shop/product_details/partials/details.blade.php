            ...
            <div class="btn-wrapper mt-4">
                @if(!empty($product->preview_link))
                    <a href="{{$product->preview_link}}"
                       class="cmn-btn cmn-btn-outline-one cmn-btn-small color-one radius-0 mt-2 pdf_preview"
                ...
                <ul class="book-details-product-contents-category-list">
                    <li class="book-details-product-contents-category-list-item">
                        ...
                        <a class="fw-600 color-heading"
                           href="{{dynamicRoute($product?->category?->slug)}}"> {{$product?->category?->name}} </a>
                    </li>
                    ...
                    @if(!empty($product->subCategory))
                        <li class="book-details-product-contents-category-list-item">
                            ...
                            <a class="fw-600 color-heading"
                               href="{{dynamicRoute($product?->subCategory?->slug)}}"> {{$product?->subCategory?->name}} </a>
                        </li>
                    @endif
                    ...
                    @if(count($product->childCategory) > 0)
                        <li class="book-details-product-contents-category-list-item">
                            ...
                            @foreach($product?->childCategory ?? [] as $child_category)
                                <a class="fw-600 color-heading"
                                   href="{{dynamicRoute($child_category?->slug)}}"> {{$child_category->name}} </a>{{!$loop->last ? ',' : ''}}
                            @endforeach
                        </li>
                    @endif
                    ...
                    @if(!empty($product->tag))
                        <li class="book-details-product-contents-category-list-item">
                            ...
                            @foreach($product->tag ?? [] as $tag)
                                <a class="fw-600 color-heading"
                                   href="{{dynamicRoute($tag->tag_name)}}"> {{$tag->tag_name}} </a>{{!$loop->last ? ',' : ''}}
                            @endforeach
                        </li>
                    @endif
                </ul>
