<template>
    <div class="admin-form-page">
        ...
        <form @submit.prevent="submitForm" class="form-container">
            ...
            <FormInput v-model="form.password" type="password" label="Password" required />
            ...
            <div class="form-actions">
                <button type="submit" class="btn-primary" :disabled="loading">Create Admin</button>
...
export default {
    ...
    setup() {
        ...
        const submitForm = async () => {
            loading.value = true
            try {
                await api.admins.create(form.value)
                showToastMessage('success', 'Success', 'Admin created successfully')
                setTimeout(() => router.push('/admins'), 1500)
            } catch (error) {
                showToastMessage('error', 'Error', error.response?.data?.message || 'Failed to create admin')
            } finally {
                loading.value = false
            }
        }
