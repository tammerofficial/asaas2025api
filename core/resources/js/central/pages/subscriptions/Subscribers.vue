<template>
    <div class="subscribers-page">
        ...
        <div class="filters-bar">
            ...
            <select v-model="statusFilter" class="filter-select" @change="loadSubscribers(1)">
        ...
        <div class="table-container">
            ...
            <table v-else class="subscribers-table">
                ...
                <tbody>
                    ...
                    <tr v-else v-for="subscriber in subscribers" :key="subscriber.id">
                        <td>{{ subscriber.id }}</td>
                        <td>
                            <div class="user-info">
                                <div class="user-name">{{ subscriber.name || subscriber.user_name || 'N/A' }}</div>
                            </div>
                        </td>
                        <td>{{ subscriber.email || 'N/A' }}</td>
                        <td>
                            <span class="tenant-badge">{{ subscriber.tenant_id || 'N/A' }}</span>
                        </td>
                        <td>{{ subscriber.package_name || subscriber.plan_name || 'N/A' }}</td>
                        <td>
                            <StatusBadge :status="subscriber.status || 'active'" />
                        </td>
                        <td>{{ formatDate(subscriber.expires_at || subscriber.expire_date) }}</td>
        ...
        <!-- Pagination -->
        <div v-if="!loading && subscribers.length > 0 && pagination.last_page > 1" class="pagination">
            <button 
...
const loadSubscribers = async (page = 1) => {
    ...
    try {
        ...
        const response = await api.subscriptions.subscribers(params)
...
const viewSubscriber = (subscriber) => {
    // Navigate to subscriber details if route exists
    console.log('View subscriber:', subscriber)
}

const deleteSubscriber = async (subscriber) => {
    if (!confirm(`Are you sure you want to delete subscriber "${subscriber.name || subscriber.email}"?`)) {
        return
    }
    
    try {
        // API call to delete subscriber
