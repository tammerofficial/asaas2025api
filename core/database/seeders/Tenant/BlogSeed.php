...
class BlogSeed extends Seeder
{
    ...
    private function blog_category_seed()
    {
        ...
        BlogCategory::all()->each(function (BlogCategory $category) {
    ...
    private function blog_tag_seed()
    {
        ...
        BlogTag::all()->each(function (BlogTag $tag) {
            try {
                $tag->slug()->create(['slug' => $tag->slug]);
            } catch (\Exception $exception) {
                $tag->slug()->create(['slug' => create_slug($tag->slug, 'Slug')]);
            }
        });
    }
    ...
    private function blog_store($title, $description, $category_id, $image_id)
    {
        ...
        $blog = new Blog();
        $blog->title =  esc_html($title);
        $blog->blog_content = $description;
        $blog->excerpt = esc_html('blog excerpt');
        ...
        $Metas = [
            'title' => esc_html('blog'),
            'description' => esc_html('blog'),
            'image' => null,
            //twitter
            'tw_image' => null,
            'tw_title' => esc_html('blog'),
            'tw_description' => esc_html('blog'),
            //facebook
            'fb_image' => null,
            'fb_title' =>esc_html('blog'),
            'fb_description' => esc_html('blog'),
        ];
