            ...
            <div class="col-md-12">
                <div class="row gy-4 d-flex align-items-start">
                    <div class="col-xxl-2 col-xl-3 col-lg-12">
                        <div class="nav flex-column nav-pills border-1 radius-10 me-3" id="product-create-step" role="tablist"
...
@section('scripts')
            ...
            <script>
                ...
                $(document).ready(function () {
                    ...
                    // disable tab if required fields not filled
                    // $(document).ready(function () {
                    //     // let form = document.getElementById('product-create-step');
                    //     // let form_right = document.getElementById('product-create-form');
                    //     // let navLink = form.querySelectorAll('.nav-link');
                    //     // let navTab = form_right.querySelectorAll('.tab-pane');
                    //     // // $(navLink).not(':first').addClass('disabled');
                    //     // navLink.forEach((e)=>{
                    ...
                    //     // navTab.forEach((e)=> {
                    //     //     if(e.classList.contains('active')) {
                    //     //         const requiredFields = e.querySelectorAll('input[required], select[required], textarea[required]');
                    //     //         const allFilled = Array.from(requiredFields).every(field => field.value.trim() !== '');
                    //     //
                    //     //         if (allFilled) {
                    //     //             const currentTabLink = document.querySelector('#product-create-step .nav-link.active');
                    //     //             if (!currentTabLink) return;
                    //     //
                    //     //             const currentLi = currentTabLink.closest('li');
                    ...
                    //     //                     const tabEvent = new bootstrap.Tab(nextTab); // Bootstrap 5 tab instance
                    //     //                     tabEvent.show(); // Show the next tab
                    //     //                 }
                    //     //             }
                    //     //         }
                    //     //
                    ...
                    //
                    //     //======
                    //     //Automatically watch inputs inside tab-panes
                    //     $(navTab).each(function () {
                    //         const tab = $(this);
                    //
                    //         tab.find('input[required], select[required], textarea[required]').on('input change', function () {
                    //             const allFilled = tab.find('input[required], select[required], textarea[required]').filter(function () {
                    //                 return !$(this).val();
                    //             }).length === 0;
                    //
                    //             if (allFilled) {
                    //                 const currentTabLink = $('#product-create-step .nav-link.active');
                    //                 const nextTab = currentTabLink.parent().next().find('.nav-link');
                    //
                    //                 if (nextTab.length) {
                    ...
                    $(document).on("submit", "#product-create-form", function (e) {
                        ...
                        send_ajax_request("post", new FormData(e.target), $(this).attr("data-request-route"), function () {
                            ...
                            //         //     .after('<span class="text-danger">' + messages[0] + '</span>');
                            ...
                            //     });
                            //
                            //     // Jump to the tab with first error
                            //     let firstErrorInput = form.find('[name="' + firstErrorField + '"]');
                            //     let firstErrorTabId = firstErrorInput.closest('.tab-pane').attr('id');
                            //     $('a[href="#' + firstErrorTabId + '"]').removeClass('disabled').tab('show');
                            // }
                        });
                    })
