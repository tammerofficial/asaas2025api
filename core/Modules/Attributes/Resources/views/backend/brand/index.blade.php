                    ...
                    <div class="card-body">
                        <form action="{{route('tenant.admin.product.brand.manage.update')}}" method="post">
                            ...
                            <div class="form-group">
                                <label for="edit-name">{{__('Name')}}<span style="font-size: 14px; color:red;">*</span></label>
                                <input type="text" class="form-control"  id="edit-name" name="name" placeholder="{{__('Brand Name')}}">
                                <p class="text-danger error-text" id="error-edit-name"></p>
                            </div>

                            <div class="form-group">
                                <label for="edit-slug">{{__('Slug')}}<span style="font-size: 14px; color:red;">*</span></label>
                                <input type="text" class="form-control"  id="edit-slug" name="slug" placeholder="{{__('Brand Slug')}}">
                                <p class="text-danger error-text" id="error-edit-slug"></p>
                            </div>
                            ...
                            <p class="text-danger error-text" id="error-edit-logo"></p>
                ...
                <div class="modal-content">
                    ...
                    <div class="card-body">
                        <form action="{{route('tenant.admin.product.brand.manage.store')}}" method="post" enctype="multipart/form-data">
                            @csrf
                            <div class="form-group">
                                <label for="name">{{__('Name')}}<span style="font-size: 14px; color:red;">*</span></label>
                                <input type="text" class="form-control"  id="create-name" name="name" placeholder="{{__('Brand Name')}}">
                                <p class="text-danger error-text" id="error-create-name"></p>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <p class="text-danger error-text" id="error-create-slug"></p>
...
{{--                            <x-fields.media-upload :title="'Banner'" :name="'banner_id'" :dimentions="'200x200'"/>--}}
                            ...
                            @if(!empty(get_static_option('tenant_admin_dashboard_theme') ) && get_static_option('tenant_admin_dashboard_theme') == 'metronic')
                                ...
                                <p class="text-danger error-text" id="error-create-logo"></p>
...
@section('scripts')
    ...
    <x-table.btn.swal.js />
    <x-media-upload.js/>
{{--    <x-bulk-action.js :route="route('tenant.admin.brands.bulk.action')" />--}}
    <x-bulk-action.brand-js :url="route('tenant.admin.brands.bulk.action')"/>
    ...
    <script>
        ...
        $(document).ready(function () {
            showError('error-edit-name', 'Name is required');
            showError('error-edit-slug', 'Slug is required');
            showError('error-edit-logo', 'Logo is required');
            ...
            $('#brand_manage_create_modal form').on('submit', function (e) {
                let isValid = true;
                clearErrors($('#brand_manage_create_modal'));

                let name = $('#create-name').val().trim();
                let slug = $('#create-slug').val().trim();
                let logo = $('#brand_manage_create_modal').find('input[name="image_id"]').val();

                if (name === '') {
                    isValid = false;
                    showError('error-create-name', 'Name is required');
                }

                if (slug === '') {
                    isValid = false;
                    showError('error-create-slug', 'Slug is required');
                }

                if (!logo) {
                    isValid = false;
                    showError('error-create-logo', 'Logo is required');
                }
            ...
            $('#brand_manage_edit_modal form').on('submit', function (e) {
                let isValid = true;
                clearErrors($('#brand_manage_edit_modal'));

                let name = $('#edit-name').val().trim();
                let slug = $('#edit-slug').val().trim();
                let logo = $('#brand_manage_edit_modal').find('input[name="image_id"]').val();
