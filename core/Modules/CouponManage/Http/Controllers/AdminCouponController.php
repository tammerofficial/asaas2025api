...
class AdminCouponController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        $updated = ProductCoupon::find($request->id)->update([
            ...
            'minimum_spend' => $request->minimum_spend ?? null,
            'maximum_spend' => $request->maximum_spend ?? null,
            'usage_limit_per_coupon' =>$request->usage_limit_per_coupon ?? null,
            'usage_limit_per_user' => $request->usage_limit_per_user ?? null,
        ]);
     ...
     * @return \Illuminate\Http\RedirectResponse
     */
    public function destroy(ProductCoupon $item): JsonResponse
    {
        try {
            $item->delete();
            ...
            return response()->json([
                'type' => 'success',
                'msg' => __('Product Coupon deleted successfully.')
            ]);
        } catch (\Exception $e) {
            return response()->json([
                'type' => 'error',
                'msg' => __('Failed to delete Product Coupon.')
            ], 500);
        }
